...
function ClerkMode(props){
  const {event,regs,members,families,addReg,updateReg,promoteFromWaitlist,submitApproval,approvals,user,logout,activeCount,isFull,wlRegs,exRegs,lang,setLang,pendingApprovals,theme,toggleTheme}=props;
  const t=useT();
  ...
  const viewRegs=(tab==="active"?allActive:tab==="waitlist"?wlRegs:regs.filter(r=>r.eventId===event?.id&&r.cancelled))
    .filter(r=>r.memberName.toLowerCase().includes(search.toLowerCase())||r.regNumber.toLowerCase().includes(search.toLowerCase())||r.church.toLowerCase().includes(search.toLowerCase()));
    ...
    <div className="app-shell">
      ...
      <div className="main-scroll">
        <div className="page-pad">
            ...
            <div style={{background:"#fffbeb",border:"1.5px solid #f59e0b",borderRadius:10,padding:"12px 16px",marginBottom:14}}>
              ...
              {myPending.map(a=>(
                <div key={a.id} style={{fontSize:13,display:"flex",justifyContent:"space-between",padding:"4px 0",borderBottom:"1px solid #fde68a",flexWrap:"wrap",gap:4}}>
                  <span>{a.type==="capacity_override"?"⚡":"💸"} <strong>{a.memberName}</strong> ({a.category})</span>
          ...
          {event?.paymentDeadlineDays&&(()=>{
            const overdue=allActive.filter(r=>!isDeadlineExempt(r,allActive)&&daysSince(r.registeredAt)>=event.paymentDeadlineDays);
            const urgent=allActive.filter(r=>!isDeadlineExempt(r,allActive)&&daysSince(r.registeredAt)>=event.paymentDeadlineDays-2&&daysSince(r.registeredAt)<event.paymentDeadlineDays);
            if(!overdue.length&&!urgent.length) return null;
            return(
              <div style={{marginBottom:14}}>
                {overdue.length>0&&<div style={{background:"#fee2e2",border:"1px solid #c0392b",borderRadius:8,padding:"10px 14px",fontSize:13,color:"#991b1b",marginBottom:8}}>
                  ⏰ <strong>{overdue.length}</strong> {lang==="en"?"registration(s) past payment deadline (team members excluded):":"inscrição(ões) com prazo expirado (equipes de serviço excluídas):"}
                  <button className="btn btn-sm" style={{marginLeft:12,background:"#c0392b",color:"#fff"}} onClick={()=>{
                    overdue.forEach(r=>updateReg(r.id,{waitlisted:true,waitlistReason:"Pagamento não realizado no prazo"},{status:"Em Espera",note:"Prazo de pagamento expirado"}));
                    if(wlRegs.length>0) notify(`${overdue.length} vaga(s) liberada(s). ${wlRegs.length} em espera — confirme manualmente.`);
                  }}>{lang==="en"?"Move all":"Mover todos"}</button>
                </div>}
                {urgent.length>0&&<div style={{background:"#fef3c7",border:"1px solid #d4820a",borderRadius:8,padding:"10px 14px",fontSize:13,color:"#92400e"}}>
                  ⚠️ <strong>{urgent.length}</strong> {lang==="en"?"registration(s) expiring soon (≤2 days)":"inscrição(ões) vencendo em breve (≤2 dias)"}
                  <div style={{fontSize:11,marginTop:4,opacity:.8}}>{lang==="en"?"Team members and exempt registrants are not affected.":"Membros de equipes de serviço e isentos não são afetados."}</div>
                </div>}
              </div>
            );
          })()}
          ...
          <div style={{display:"flex",gap:10,marginBottom:10}}>
            <div className="sb" style={{flex:1}}><span className="si-icon">🔍</span>
          ...
          {suggestions.length>0&&(
            <div style={{background:"#fff",border:"1.5px solid var(--border)",borderRadius:10,marginBottom:10,overflow:"hidden"}}>
              ...
              {suggestions.map(m=>(
                <div key={m.id} onClick={()=>{setPrefill(m);setShowReg(true);}}
                  style={{padding:"9px 14px",borderTop:"1px solid var(--border)",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center"}}
          ...
          <div className="card" style={{padding:0,overflow:"hidden"}}>
            <div style={{padding:"11px 16px",borderBottom:"1px solid var(--border)",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
              <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
                {[[{k:"active",l:`${t.activeTab} (${allActive.length})`},{k:"waitlist",l:`${t.waitlistTab} (${wlRegs.length})`},{k:"cancelled",l:t.cancelledTab}]].flat().map(({k,l})=>(
      ...
      {showReg&&<RegModal event={event} members={members} families={families} isFull={isFull} existingRegs={allActive} prefill={prefill}
        onClose={()=>{setShowReg(false);setPrefill(null);}}
        onSave={d=>{addReg(d);setShowReg(false);setPrefill(null);}}
