...
function ClerkMode(props) {
  ...
  const myPending = approvals.filter(
    (a) => a.eventId === event?.id && a.requestedById === user?.id && a.status === "pending"
  );
  ...
  const viewRegs = (
    tab === "active"
      ...
      : tab === "waitlist"
        ? wlRegs
        : regs.filter((r) => r.eventId === event?.id && r.cancelled)
  ).filter(
    (r) =>
      ...
      ? members
          .filter(
            (m) =>
              ...
              !allActive.find((r) => r.memberId === m.id)
          )
          .slice(0, 5)
      : [];
        ...
        <div className="page-pad">
          {myPending.length > 0 && (
            ...
            >
              ...
              {myPending.map((a) => (
                ...
                >
                  <span>
                    {a.type === "capacity_override" ? "⚡" : "💸"} <strong>{a.memberName}</strong> (
                    {a.category})
                  </span>
          ...
          {event?.paymentDeadlineDays &&
            (() => {
              const overdue = allActive.filter(
                (r) =>
                  !isDeadlineExempt(r, allActive) &&
                  daysSince(r.registeredAt) >= event.paymentDeadlineDays
              );
              const urgent = allActive.filter(
                (r) =>
                  !isDeadlineExempt(r, allActive) &&
                  daysSince(r.registeredAt) >= event.paymentDeadlineDays - 2 &&
                  daysSince(r.registeredAt) < event.paymentDeadlineDays
              );
                ...
                <div style={{ marginBottom: 14 }}>
                  {overdue.length > 0 && (
                    ...
                    >
                      ...
                      {lang === "en"
                        ? "registration(s) past payment deadline (team members excluded):"
                        : "inscrição(ões) com prazo expirado (equipes de serviço excluídas):"}
                      <button
                        className="btn btn-sm"
                        style={{ marginLeft: 12, background: "#c0392b", color: "#fff" }}
                        onClick={() => {
                          overdue.forEach((r) =>
                            updateReg(
                              r.id,
                              {
                                waitlisted: true,
                                waitlistReason: "Pagamento não realizado no prazo",
                              },
                              { status: "Em Espera", note: "Prazo de pagamento expirado" }
                            )
                          );
                          if (wlRegs.length > 0)
                            notify(
                              `${overdue.length} vaga(s) liberada(s). ${wlRegs.length} em espera — confirme manualmente.`
                            );
                        }}
                      >
                        {lang === "en" ? "Move all" : "Mover todos"}
                      </button>
                    </div>
                  )}
                  {urgent.length > 0 && (
                    ...
                    >
                      ⚠️ <strong>{urgent.length}</strong>{" "}
                      {lang === "en"
                        ? "registration(s) expiring soon (≤2 days)"
                        : "inscrição(ões) vencendo em breve (≤2 dias)"}
                      <div style={{ fontSize: 11, marginTop: 4, opacity: 0.8 }}>
                        {lang === "en"
                          ? "Team members and exempt registrants are not affected."
                          : "Membros de equipes de serviço e isentos não são afetados."}
                      </div>
                    </div>
                  )}
                </div>
          ...
          <div className="stat-grid-4" style={{ marginBottom: 18 }}>
            {[
              ...
              {
                label: t.paid,
                value: allActive.filter((r) => r.paid || r.exempt).length,
                color: "#2d8a4e",
              },
              {
                label: t.pending,
                ...
                <tbody>
                  {viewRegs.length === 0 && (
                    <tr>
                      <td
                        colSpan={8}
                        style={{ textAlign: "center", color: "#6b7280", padding: 28 }}
                      >
                        {t.noRecords}
                      </td>
                    </tr>
                  ...
                  {viewRegs.map((r) => (
                    <tr key={r.id}>
                      ...
                      <td>
                        <StatusBadge r={r} event={event} allRegs={allActive} />
                      </td>
                      <td>
                        <div style={{ display: "flex", gap: 4 }}>
                          ...
                          {!r.waitlisted && !r.paid && !r.exempt && !r.cancelled && (
                            <button
                              className="btn btn-ok btn-sm"
                              onClick={() => updateReg(r.id, { paid: true })}
                            >
                              {t.markPaid}
                            </button>
                          )}
