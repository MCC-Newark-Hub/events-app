...
function TeamLeaderView(props){
  const {event,regs,members,rosters,setRosters,user,logout,notify,lang,setLang,theme,toggleTheme}=props;
  const t=useT();
  ...
  const addToRoster=(team,mid)=>{setRosters(prev=>{const ex=prev.find(r=>r.eventId===event?.id&&r.team===team);if(ex){if(ex.memberIds.includes(mid)) return prev;return prev.map(r=>r.eventId===event?.id&&r.team===team?{...r,memberIds:[...r.memberIds,mid]}:r);}return [...prev,{eventId:event?.id,team,memberIds:[mid]}];});notify("✓ Added!");};
  const removeFromRoster=(team,mid)=>{setRosters(prev=>prev.map(r=>r.eventId===event?.id&&r.team===team?{...r,memberIds:r.memberIds.filter(x=>x!==mid)}:r));notify("Removed.");};
  const searchR=msearch.length>1?members.filter(m=>m.name.toLowerCase().includes(msearch.toLowerCase())).slice(0,6):[];
  return(
    <div className="app-shell">
      <Topbar title={t.teamTitle} sub={event?.name} user={user} logout={logout} lang={lang} setLang={setLang} theme={theme} toggleTheme={toggleTheme}/>
      <div className="main-scroll"><div className="page-pad">
        <div style={{marginBottom:16}}>
          <h2 style={{fontFamily:"'Lora',Georgia,serif",fontSize:22}}>{t.hello}, {user?.name}!</h2>
        ...
        {myTeams.map(team=>{
          ...
          const teamMembers=mids.map(mid=>members.find(m=>m.id===mid)).filter(Boolean);
          ...
          return(<div key={team} style={{marginBottom:24}}>
            ...
            <div className="card" style={{padding:0,overflow:"hidden"}}>
              <div style={{padding:"11px 16px",background:"#f8f9fb",borderBottom:"1px solid var(--border)",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
              ...
              {isEditing&&(<div style={{padding:"10px 14px",background:"#fffbeb",borderBottom:"1px solid var(--border)"}}>
              ...
              {teamMembers.length===0?(<p style={{padding:24,color:"#6b7280",textAlign:"center"}}>{t.noMembers}</p>):(
                <div className="table-wrap"><table className="table">
                  <thead><tr><th>{t.memberName}</th><th>{t.cat}</th><th>{t.churchH}</th><th>{t.regH}</th><th></th></tr></thead>
                  <tbody>{teamMembers.map(m=>{const s=getStatus(m.id);const cfg=STATUS_CFG[s];return(<tr key={m.id}>
                    <td style={{fontWeight:600}}><span className={`dot ${cfg.dot}`}></span>{m.name}</td>
                    <td><span className="badge badge-blue">{m.category}</span></td>
                    <td style={{fontSize:12,color:"#6b7280"}}>{m.church}</td>
                    <td><span className={`badge ${cfg.badge}`} style={{fontSize:10}}>{t[s==="not_registered"?"notRegistered":s==="pending"?"pendPayment":"confirmed"]}</span></td>
                    <td><button onClick={()=>removeFromRoster(team,m.id)} style={{background:"none",border:"none",cursor:"pointer",color:"#9ca3af",fontSize:18,lineHeight:1}}>×</button></td>
                  </tr>);})}
                  </tbody>
                </table></div>
              )}
            </div>
            {notReg.length>0&&<div style={{marginTop:8,padding:"10px 14px",background:"#fef3c7",borderRadius:8,fontSize:13,color:"#92400e"}}>⚠️ <strong>{notReg.length} {t.notRegisteredWarn}:</strong> {notReg.map(m=>m.name).join(", ")} — {t.reachOut}</div>}
          </div>);
        })}
        <div style={{padding:"10px 14px",background:"#fff",borderRadius:10,border:"1px solid var(--border)",fontSize:12,color:"#6b7280",display:"flex",gap:16,flexWrap:"wrap"}}>
