        ...
        {sec==="dashboard"&&(
          <div>
            ...
            <div className="two-col" style={{marginBottom:14}}>
              <div className="card"><h4 style={{fontWeight:700,marginBottom:10}}>{t.category}</h4>{byCat.map(x=><div key={x.c} style={{display:"flex",justifyContent:"space-between",padding:"7px 0",borderBottom:"1px solid var(--border)"}}><span style={{fontSize:14}}>{x.c}</span><span className="badge badge-blue">{x.n}</span></div>)}</div>
              <div className="card"><h4 style={{fontWeight:700,marginBottom:10}}>{t.church}</h4>{byCh.map(x=><div key={x.ch} style={{display:"flex",justifyContent:"space-between",padding:"7px 0",borderBottom:"1px solid var(--border)"}}><span style={{fontSize:13}}>{x.ch}</span><div style={{display:"flex",gap:5}}><span className="badge badge-green">{x.paid}✓</span><span className="badge badge-blue">{x.total}</span></div></div>)}</div>
            </div>
            <div className="card">
              ...
              <div style={{background:"#e5e7eb",borderRadius:99,height:10,overflow:"hidden"}}><div style={{background:"#2d8a4e",height:"100%",width:`${pct}%`,borderRadius:99}}/></div>
