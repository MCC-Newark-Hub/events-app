        ...
        <div key={a.id} className={`apr-card ${a.type === "exemption" ? "danger" : ""}`}>
          ...
          >
            <span style={{ fontWeight: 700, fontSize: 14 }}>
              {a.type === "capacity_override" ? t.capacityOverride : t.exemptionReq}
            </span>
            <span style={{ fontSize: 12, color: "#6b7280" }}>
              {t.requestedBy} {a.requestedBy}
            </span>
          </div>
          <div
            style={{
          ...
          >
            ...
            {a.type === "capacity_override" && (
              <div style={{ color: "#c4390a", fontWeight: 600, marginTop: 4 }}>
                Registration #{activeCount + 1} — above capacity of {event?.capacity}
              </div>
            )}
            {a.type === "exemption" && (
          ...
          <div className="fr">
            <button
              ...
              onClick={() => resolveApproval(a.id, true, note[a.id] || "")}
            >
              {t.approve}
            </button>
            <button
              className="btn btn-danger"
              onClick={() => resolveApproval(a.id, false, note[a.id] || "")}
            >
              {t.deny}
            </button>
          </div>
            ...
            >
              <div>
                ...
                <span style={{ marginLeft: 8, color: "#6b7280" }}>
                  {a.type === "capacity_override" ? t.excedente : t.exempt}
                </span>
              </div>
              <span className={a.status === "approved" ? "badge badge-green" : "badge badge-red"}>
                {a.status === "approved" ? t.approved : t.denied}
              </span>
            </div>
          ))}
        </div>
