...
export default function ApprovalsPanel({ approvals, resolveApproval, event, activeCount }) {
  ...
  return (
    <div>
      ...
      {pending.map((a) => (
        <div key={a.id} className={`apr-card ${a.type === "exemption" || a.type === "late_registration" ? "danger" : ""}`}>
          <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 6, marginBottom: 10 }}>
            <span style={{ fontWeight: 700, fontSize: 14 }}>{a.type === "capacity_override" ? t.capacityOverride : a.type === "late_registration" ? t.lateRegistrationReq : t.exemptionReq}</span>
        ...
        <div>
          ...
          {resolved.map((a) => (
            <div key={a.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 14px", background: "#fff", borderRadius: 8, marginBottom: 6, border: "1px solid var(--border)", fontSize: 13 }}>
              <div><span style={{ fontWeight: 600 }}>{a.memberName}</span><span style={{ marginLeft: 8, color: "#6b7280" }}>{a.type === "capacity_override" ? t.excedente : a.type === "late_registration" ? t.lateRegistrationReq : t.exempt}</span></div>
